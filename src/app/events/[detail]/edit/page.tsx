...
export default function EventCreate() {
  ...
  const { event, loading, error } = useGetEventDetail(eventSlug);
  ...
  React.useEffect(() => {
    if (event) {
      ...
      setValue("timezone", event.timezone || "GMT+07:00");
      setValue("format", event.format);
      setValue("address", event.address);
      setValue("location_map", event.location_map);
      setValue("isFree", event.isFree);
      setValue("content", event.content || "");
      setValue("thumbnail_url", event.image);
      setValue("bank_account", event.bank_account || "");
      setValue("account_number", event.account_number || "");
      setValue("account_name", event.account_name || "");
      setValue("ticket_price", event.ticket_price || "");
      setValue("type_actifity", event.type_actifity || "");
      setValue("communityId", event.community._id || "");
      setValue("capacity", event.capacity || "");
      setValue("is_active", event.is_active || false);
      setValue("isApprovalRequired", event.isApprovalRequired || false);

      if (event.image) {
        setImagePreview(event.image);
      }

      if (event.hostDetails && event.hostDetails.length > 0) {
        setValue(
          "hosts",
          event.hostDetails.map((host: { _id: string; full_name: string }) => ({
            value: host._id,
      ...
      if (
        event.collaboratingCommunityDetails &&
        event.collaboratingCommunityDetails.length > 0
      ) {
        setValue(
          "collaboratingCommunities",
          event.collaboratingCommunityDetails.map(
            (collaborate: { _id: string; name: string }) => ({
              value: collaborate._id,
              label: collaborate.name,
            })
          )
        );
      }
  ...
  const onSubmit = async (data: EventFormInputs) => {
    ...
    try {
      ...
      const formDataWithImage: IEventInput = {
        ...
        collaboratingCommunities: Array.isArray(data.collaboratingCommunities)
          ? data.collaboratingCommunities.map((c: any) =>
              typeof c === "string" ? c : c.value
            )
          : [],
      };

      await updateEvent(formDataWithImage);
      toast({
        variant: "default",
        title: "Berhasil",
        description: "Event Anda telah berhasil diperbarui.",
      });
    } catch (error) {
      toast({
        variant: "destructive",
        title: "Peringatan",
        description: "Event Anda tidak dapat diperbaharui. Silakan coba lagi.",
      });
  ...
  const initialContent: JSONContent | undefined = watch("content")
                ...
                <div className="grid grid-cols-1 gap-4">
                      ...
                      <div className="flex-1 overflow-y-auto p-2">
                        <TiptapEditorInput
                          onChange={(value: JSONContent) => {
                            setValue("content", JSON.stringify(value));
                          }}
                  ...
                  <div>
                    ...
                    <Card className="bg-gray-100 rounded-xl space-y-4 shadow-none border-none">
                      <CardContent className="p-3 space-y-4 py-4">
                        ...
                        <div className="flex items-center justify-between w-full">
                          <p className="text-green-700">Required Approval</p>
                          <div className="flex items-center space-x-2">
                            <Controller
                              ...
                              render={({ field }) => (
                                <Switch
                                  checked={field.value}
                                  onCheckedChange={field.onChange}
                                  className={`data-[state=checked]:bg-green-600 bg-gray-200 border-transparent transition-colors inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full p-0.5`}
                                />
                              )}
                            />
                          </div>
                  ...
                  <div className="mt-4">
                    <Button
                      ...
                      disabled={isLoading}
                    >
                      {isLoading ? "Updating Event..." : "Updating Event"}
                    </Button>
                  </div>
                </div>
              </div>
